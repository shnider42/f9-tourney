...
Usage:
  python apply_f9_theme_pro.py
  python apply_f9_theme_pro.py --logo path/to/logo.png
"""
...
def wrap_first_form(html:str)->str:
    # Put the first <form ...> in a .panel
    m = re.search(r"<form[^>]*>", html, flags=re.I)
    if not m: return html
    start = m.start()
    end = m.end()
    if '<div class="panel' in html[:start]:  # crude guard if already wrapped
        return html
    return html[:start] + '<div class="panel mb-4">' + html[start:]            .replace("</form>", "</form></div>", 1)

def wrap_roster_columns(html:str)->str:
    # After each col-md-4 opening div for roster lists, insert panel
    return html.replace('<div class="col-md-4">\n      ', '<div class="col-md-4">\n      <div class="panel roster">')                .replace('      </ol>\n    </div>', '      </ol>\n      </div>\n    </div>')

def add_tags_to_items(html:str)->str:
    # Append a small tag showing rank/region; leave original info intact
...
def main():
    ...
    # Copy theme assets if present next to script
